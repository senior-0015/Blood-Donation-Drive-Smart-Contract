...
fn stable_b_tree_map_contains_key(
    ...
    match memory_id {
        ...
        1u8 => {
            let wrapped_key = &StableBTreeMap1KeyType(
                key_js_value.try_from_vm_value(&mut *context)?,
            );
        ...
        2u8 => {
            let wrapped_key = &StableBTreeMap2KeyType(
                key_js_value.try_from_vm_value(&mut *context)?,
            );
        ...
        3u8 => {
            let wrapped_key = &StableBTreeMap3KeyType(
                key_js_value.try_from_vm_value(&mut *context)?,
            );
...
fn stable_b_tree_map_insert(
    ...
    match memory_id {
        ...
        1u8 => {
            let key = StableBTreeMap1KeyType(
                key_js_value.try_from_vm_value(&mut *context)?,
            );
            let value = StableBTreeMap1ValueType(
                value_js_value.try_from_vm_value(&mut *context)?,
            );
            STABLE_B_TREE_MAP_1_REF_CELL
                .with(|stable_b_tree_map_ref_cell| {
        ...
        2u8 => {
            let key = StableBTreeMap2KeyType(
                key_js_value.try_from_vm_value(&mut *context)?,
            );
            let value = StableBTreeMap2ValueType(
                value_js_value.try_from_vm_value(&mut *context)?,
            );
            STABLE_B_TREE_MAP_2_REF_CELL
                .with(|stable_b_tree_map_ref_cell| {
        ...
        3u8 => {
            let key = StableBTreeMap3KeyType(
                key_js_value.try_from_vm_value(&mut *context)?,
            );
            let value = StableBTreeMap3ValueType(
                value_js_value.try_from_vm_value(&mut *context)?,
            );
            STABLE_B_TREE_MAP_3_REF_CELL
                .with(|stable_b_tree_map_ref_cell| {
...
fn stable_b_tree_map_items(
    ...
    match memory_id {
        ...
        1u8 => {
            let items = STABLE_B_TREE_MAP_1_REF_CELL
                .with(|stable_b_tree_map_ref_cell| {
                    stable_b_tree_map_ref_cell
                        .borrow()
                        .iter()
        ...
        2u8 => {
            let items = STABLE_B_TREE_MAP_2_REF_CELL
                .with(|stable_b_tree_map_ref_cell| {
                    stable_b_tree_map_ref_cell
                        .borrow()
                        .iter()
        ...
        3u8 => {
            let items = STABLE_B_TREE_MAP_3_REF_CELL
                .with(|stable_b_tree_map_ref_cell| {
                    stable_b_tree_map_ref_cell
                        .borrow()
                        .iter()
...
thread_local! {
    ...
    .with(| m | { m.borrow()
    .get(ic_stable_structures::memory_manager::MemoryId::new(0u8)) }),)); static
    STABLE_B_TREE_MAP_1_REF_CELL : std::cell::RefCell <
    ic_stable_structures::StableBTreeMap < StableBTreeMap1KeyType,
    StableBTreeMap1ValueType, ic_stable_structures::memory_manager::VirtualMemory <
    ic_stable_structures::DefaultMemoryImpl > > > =
    std::cell::RefCell::new(ic_stable_structures::StableBTreeMap::init(MEMORY_MANAGER_REF_CELL
    .with(| m | { m.borrow()
    .get(ic_stable_structures::memory_manager::MemoryId::new(1u8)) }),)); static
    STABLE_B_TREE_MAP_2_REF_CELL : std::cell::RefCell <
    ic_stable_structures::StableBTreeMap < StableBTreeMap2KeyType,
    StableBTreeMap2ValueType, ic_stable_structures::memory_manager::VirtualMemory <
    ic_stable_structures::DefaultMemoryImpl > > > =
    std::cell::RefCell::new(ic_stable_structures::StableBTreeMap::init(MEMORY_MANAGER_REF_CELL
    .with(| m | { m.borrow()
    .get(ic_stable_structures::memory_manager::MemoryId::new(2u8)) }),)); static
    STABLE_B_TREE_MAP_3_REF_CELL : std::cell::RefCell <
    ic_stable_structures::StableBTreeMap < StableBTreeMap3KeyType,
    StableBTreeMap3ValueType, ic_stable_structures::memory_manager::VirtualMemory <
    ic_stable_structures::DefaultMemoryImpl > > > =
    std::cell::RefCell::new(ic_stable_structures::StableBTreeMap::init(MEMORY_MANAGER_REF_CELL
    .with(| m | { m.borrow()
...
struct StableBTreeMap0ValueType(Hospital);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap1ValueType(Patient);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap2ValueType(Donor);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap3KeyType(String);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap3ValueType(Pledge);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
#[ic_cdk_macros::query(name = "getHospitals")]
#[candid::candid_method(query, rename = "getHospitals")]
async fn _cdk_user_defined_getHospitals() -> (_AzleResult<Vec<Hospital>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getHospitals".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
...
#[ic_cdk_macros::query(name = "getPatients")]
#[candid::candid_method(query, rename = "getPatients")]
async fn _cdk_user_defined_getPatients() -> (_AzleResult<Vec<Patient>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getPatients".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getPatients", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getPatients"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getDonors")]
#[candid::candid_method(query, rename = "getDonors")]
async fn _cdk_user_defined_getDonors() -> (_AzleResult<Vec<Donor>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getDonors".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getDonors", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getDonors"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getPledgesForPatient")]
#[candid::candid_method(query, rename = "getPledgesForPatient")]
async fn _cdk_user_defined_getPledgesForPatient(
    _cdk_user_defined_patient_id: String,
) -> (_AzleResult<Vec<Pledge>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getPledgesForPatient".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getPledgesForPatient", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getPledgesForPatient"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "initBloodDrive")]
#[candid::candid_method(update, rename = "initBloodDrive")]
async fn _cdk_user_defined_initBloodDrive() -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "initBloodDrive".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("initBloodDrive", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "initBloodDrive"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::update(name = "addPatient")]
#[candid::candid_method(update, rename = "addPatient")]
async fn _cdk_user_defined_addPatient(
    _cdk_user_defined_name: String,
    _cdk_user_defined_blood_type: String,
) -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "addPatient".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("addPatient", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addPatient"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "addDonor")]
#[candid::candid_method(update, rename = "addDonor")]
async fn _cdk_user_defined_addDonor(
    _cdk_user_defined_name: String,
    _cdk_user_defined_blood_type: String,
) -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "addDonor".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("addDonor", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addDonor"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "makePledge")]
#[candid::candid_method(update, rename = "makePledge")]
async fn _cdk_user_defined_makePledge(
    _cdk_user_defined_payload: PledgePayload,
) -> (PledgeResponse) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "makePledge".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("makePledge", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "makePledge"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updatePatient")]
#[candid::candid_method(update, rename = "updatePatient")]
async fn _cdk_user_defined_updatePatient(
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updatePatient".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updatePatient", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updatePatient"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateDonor")]
#[candid::candid_method(update, rename = "updateDonor")]
async fn _cdk_user_defined_updateDonor(
    _cdk_user_defined_id: String,
    _cdk_user_defined_name: String,
    _cdk_user_defined_blood_type: String,
) -> (String) {
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateDonor".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateDonor", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateDonor"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "deletePatient")]
#[candid::candid_method(update, rename = "deletePatient")]
async fn _cdk_user_defined_deletePatient(_cdk_user_defined_id: String) -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "deletePatient".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("deletePatient", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deletePatient"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "deleteDonor")]
#[candid::candid_method(update, rename = "deleteDonor")]
async fn _cdk_user_defined_deleteDonor(_cdk_user_defined_id: String) -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "deleteDonor".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("deleteDonor", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deleteDonor"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updatePledge")]
#[candid::candid_method(update, rename = "updatePledge")]
async fn _cdk_user_defined_updatePledge(
    _cdk_user_defined_id: String,
    _cdk_user_defined_amount: String,
) -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updatePledge".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updatePledge", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updatePledge"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "deletePledge")]
#[candid::candid_method(update, rename = "deletePledge")]
async fn _cdk_user_defined_deletePledge(_cdk_user_defined_id: String) -> (String) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "deletePledge".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("deletePledge", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deletePledge"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
